...
use pontorus\event\server\QueryRegenerateEvent;
...
use pontorus\lang\BaseLang;
use pontorus\command\CommandReader;
use pontorus\network\Network;
use pontorus\network\query\QueryHandler;
use pontorus\network\RakLibInterface;
use pontorus\network\rcon\RCON;
use pontorus\network\pontorusInterface;
use pontorus\network\upnp\UPnP;
...
use pontorus\plugin\ScriptPluginLoader;
use pontorus\scheduler\ServerScheduler;
use pontorus\utils\Config;
use pontorus\utils\MainLogger;
use pontorus\utils\ServerException;
use pontorus\utils\Terminal;
use pontorus\utils\TextFormat;
use pontorus\utils\Utils;
use pontorus\utils\VersionString;
class Server{
    /** @var Server */
    ...
    private $identifiers = [];
    /** @var pontorusInterface */
    private $pontorusInterface;
    /** @var Client[] */
    private $clients = [];
    private $clientData;
    /*
    ...
    public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath){
        ...
        try{
            ...
            $version = new VersionString($this->getpontorusVersion());
            ...
            $this->logger->info($this->getLanguage()->translateString("pontorus.server.start", [TextFormat::AQUA . $this->getVersion() . TextFormat::WHITE]));
            ...
            define('pontorus\DEBUG', 2);
            if(\pontorus\DEBUG >= 0){
                @cli_set_process_title($this->getName() . " " . $this->getpontorusVersion());
            }
            $this->logger->info($this->getLanguage()->translateString("pontorus.server.networkStart", [$this->getIp() === "" ? "*" : $this->getIp(), $this->getPort()]));
            ...
            $this->logger->info($this->getLanguage()->translateString("pontorus.server.info", [
                $this->getName(),
                $this->getpontorusVersion(),
                $this->getCodename(),
                $this->getApiVersion()
            ]));
            $this->logger->info($this->getLanguage()->translateString("pontorus.server.license", [$this->getName()]));
            Timings::init();
            ...
            $this->pontorusInterface = new pontorusInterface($this, $this->getIp(), $this->getpontorusPort());
    ...
    public function getpontorusPort() : int{
        return $this->getConfig("pontorus-port", 10305);
    }
    ...
    public function start(){
        ...
        $this->logger->info($this->getLanguage()->translateString("pontorus.server.startFinished", [round(microtime(true) - \pontorus\START_TIME, 3)]));
        if(!file_exists($this->getPluginPath() . DIRECTORY_SEPARATOR . "pontorus")){
            @mkdir($this->getPluginPath() . DIRECTORY_SEPARATOR . "pontorus");
        }
        $this->tickProcessor();
        $this->forceShutdown();
        gc_collect_cycles();
    ...
    public function forceShutdown(){
        ...
        try{
            ...
            foreach($this->clients as $client){
                ...
                $client->close("pontorus server closed");
            }
    ...
    private function tick(){
        ...
        $this->pontorusInterface->process();
        if($this->rcon !== null){
            $this->rcon->check();
        }
    ...
    public function getVersion(){
        return \pontorus\MINECRAFT_VERSION;
    }
    ...
    public function getpontorusVersion() : string{
        return \pontorus\VERSION;
    }
    public function getApiVersion(){
        return \pontorus\API_VERSION;
    }
