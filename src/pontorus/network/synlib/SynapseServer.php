...
class pontorusServer extends Thread{
	const VERSION = "0.1.0";
	...
	public function __construct(\ThreadedLogger $logger, pontorusInterface $server, \ClassLoader $loader, $port, $interface = "0.0.0.0"){
		$this->logger = $logger;
		$this->server = $server;
	...
	public function run(){
		...
		try{
			$socket = new pontorusSocket($this->getLogger(), $this->port, $this->interface);
