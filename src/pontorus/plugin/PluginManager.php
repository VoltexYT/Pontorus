...
use pontorus\command\PluginCommand;
use pontorus\command\SimpleCommandMap;
use pontorus\event\Event;
use pontorus\event\EventPriority;
use pontorus\event\HandlerList;
...
class PluginManager{
		...
		if(is_dir($directory)){
				...
				foreach(new \RegexIterator(new \DirectoryIterator($directory), $loader->getPluginFilters()) as $file){
					...
					try{
						...
						if($description instanceof PluginDescription){
							...
							if(stripos($name, "pontorus") !== false or stripos($name, "minecraft") !== false or stripos($name, "mojang") !== false){
								$this->server->getLogger()->error($this->server->getLanguage()->translateString("pontorus.plugin.loadError", [$name, "%pontorus.plugin.restrictedName"]));
								continue;
							}elseif(strpos($name, " ") !== false){
								$this->server->getLogger()->warning($this->server->getLanguage()->translateString("pontorus.plugin.spacesDiscouraged", [$name]));
							}

							if(isset($plugins[$name]) or $this->getPlugin($name) instanceof Plugin){
								$this->server->getLogger()->error($this->server->getLanguage()->translateString("pontorus.plugin.duplicateError", [$name]));
								continue;
							}
							...
							if($compatible === false){
								$this->server->getLogger()->error($this->server->getLanguage()->translateString("pontorus.plugin.loadError", [$name, "%pontorus.plugin.incompatibleAPI"]));
								continue;
							}
						...
						$this->server->getLogger()->error($this->server->getLanguage()->translateString("pontorus.plugin.fileError", [$file, $directory, $e->getMessage()]));
			...
			while(count($plugins) > 0){
				...
				foreach($plugins as $name => $file){
					if(isset($dependencies[$name])){
						foreach($dependencies[$name] as $key => $dependency){
							if(isset($loadedPlugins[$dependency]) or $this->getPlugin($dependency) instanceof Plugin){
								...
								$this->server->getLogger()->critical($this->server->getLanguage()->translateString("pontorus.plugin.loadError", [$name, "%pontorus.plugin.unknownDependency"]));
								break;
							}
						}
					...
					if(!isset($dependencies[$name]) and !isset($softDependencies[$name])){
						...
						if($plugin = $this->loadPlugin($file, $loaders) and $plugin instanceof Plugin){
							...
							$this->server->getLogger()->critical($this->server->getLanguage()->translateString("pontorus.plugin.genericLoadError", [$name]));
						}
					}
				}
				...
				if($missingDependency === true){
					...
					if($missingDependency === true){
						foreach($plugins as $name => $file){
							$this->server->getLogger()->critical($this->server->getLanguage()->translateString("pontorus.plugin.loadError", [$name, "%pontorus.plugin.circularDependency"]));
						}
						$plugins = [];
					}
				}
	...
	protected function parseYamlCommands(Plugin $plugin){
		...
		foreach($plugin->getDescription()->getCommands() as $key => $data){
			if(strpos($key, ":") !== false){
				$this->server->getLogger()->critical($this->server->getLanguage()->translateString("pontorus.plugin.commandError", [$key, $plugin->getDescription()->getFullName()]));
				continue;
			}
			if(is_array($data)){
				...
				if(isset($data["aliases"]) and is_array($data["aliases"])){
					...
					foreach($data["aliases"] as $alias){
						if(strpos($alias, ":") !== false){
							$this->server->getLogger()->critical($this->server->getLanguage()->translateString("pontorus.plugin.aliasError", [$alias, $plugin->getDescription()->getFullName()]));
							continue;
						}
						$aliasList[] = $alias;
					}
		...
		foreach($event->getHandlers()->getRegisteredListeners() as $registration){
			...
			try{
				...
				$this->server->getLogger()->critical(
					$this->server->getLanguage()->translateString("pontorus.plugin.eventError", [
						$event->getEventName(),
			...
			if(!$method->isStatic()){
				...
				if(count($parameters) === 1 and $parameters[0]->getClass() instanceof \ReflectionClass and is_subclass_of($parameters[0]->getClass()->getName(), Event::class)){
					...
					if(strpos((string) $reflection->getDocComment(), "@deprecated") !== false and $this->server->getProperty("settings.deprecated-verbose", true)){
						$this->server->getLogger()->warning($this->server->getLanguage()->translateString("pontorus.plugin.deprecatedEvent", [
							$plugin->getName(),
							$class,
